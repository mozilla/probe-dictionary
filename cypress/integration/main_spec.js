...
describe('Probe Dictionary Main Component', function() {
  it('successfully loads', function() {
    cy.visit('/');
  });

  it('auto-checks release-only checkbox upon selecting the release channel probe filter', function() {
    cy.get('#select_channel').select('release');
    cy.get('#optout').should('be.disabled').should('be.checked');
  });

  it('enables release-only checkbox upon selecting a non-release channel probe filter', function() {
    cy.get('#select_channel').select('beta');
    cy.get('#optout').should('not.be.disabled').should('be.checked');
  });

  it('sets/unsets URL search param when selecting channel probe filter', function() {
    cy.url().should('include', 'channel=beta');
    cy.get('#select_channel').select('any');
    cy.url().should('not.include', 'channel=');
  });

  it('sets/unsets URL search param when selecting version probe filter', function() {
  ...
  it('sets/unsets URL search param when selecting constraint probe filter', function() {
  ...
  it('sets/unsets URL search param when selecting search constraint', function() {
  ...
  it('opens/closes probe details and sets/unsets URL search param on probe listing click', function() {
  ...
  it('opens/closes stats view and sets/unsets URL search param on stats nav button click', function() {
  ...
  it('opens stats view when URL search param is set', function() {
    cy.visit('/?view=stats');
